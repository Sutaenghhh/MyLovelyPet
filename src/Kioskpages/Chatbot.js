...
const Chatbot = () => {
  ...
  const handleUserInput = async (e) => {
    ...
    try {
      const response = await axios.get(`http://10.10.10.110:8090/petShop/chatbot/${userNum}/${userInput}`);
  ...
  const handleStoreIssue = async (text) => {
    ...
    if (text === '종이 가방 부족') {
      ...
      try {
        const mailType = "종이";
        await axios.get(`http://10.10.10.110:8090/petShop/chatbot/${mailType}`);
